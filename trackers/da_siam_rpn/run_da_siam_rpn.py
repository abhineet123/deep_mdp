...
def tracker_eval(tracker_id, net, x_crop, target_pos, target_sz, window, scale_z, params, anchor):
    """
    ...
    :return:
    """
    raw_delta, raw_score = net(tracker_id, x_crop)
    ...
    penalty = np.exp(-(r_c * s_c - 1.) * params.penalty_k)
    score_with_penalty = penalty * score

    # window float
    score_with_penalty = score_with_penalty * (1 - params.window_influence) + window * params.window_influence
    best_pscore_id = np.argmax(score_with_penalty)

    target = delta[:, best_pscore_id] / scale_z
    target_sz = target_sz / scale_z
    lr = penalty[best_pscore_id] * score[best_pscore_id] * params.lr
    ...
    return target_pos, target_sz, score_with_penalty, score, delta, best_pscore_id
