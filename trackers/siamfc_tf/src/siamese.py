...
from trackers.siamfc_tf.src.convolutional import set_convolutional
from trackers.siamfc_tf.src.crops import extract_crops_z, extract_crops_x, pad_frame, resize_images
...
class EnvironmentParams:
    def __init__(self):
        ...
        self.root_pretrained = "trackers/siamfc_tf/pretrained"
...
def build_tracking_graph2(final_score_sz, design, env):
    ...
    :param final_score_sz:
    :param DesignParams design:
    :param EnvironmentParams env:
    :return:
    """
    ...
    frame_padded_x = tf.cast(frame_padded_x, tf.float32)
    # extract tensor of x_crops (3 scales)
    x_crops = extract_crops_x(frame_padded_x, npad_x, _pos_x_ph, _pos_y_ph, _x_sz0_ph, _x_sz1_ph, _x_sz2_ph,
                              design.search_sz)
    # use crops as input of (MatConvnet imported) pre-trained fully-convolutional Siamese net
    template_z, templates_x, p_names_list, p_val_list = _create_siamese(os.path.join(env.root_pretrained, design.net),
                                                                        x_crops, z_crops)
    ...
    templates_z = tf.stack([template_z, template_z, template_z])
    # compare templates via cross-correlation
    raw_scores = _match_templates(templates_z, templates_x, p_names_list, p_val_list)
    # upsample the score maps
    scores_up = tf.image.resize(raw_scores, [final_score_sz, final_score_sz],
                                method=tf.image.ResizeMethod.BICUBIC, align_corners=True)

    pos = [_pos_x_ph, _pos_y_ph]
    sizes = [_z_sz_ph, _x_sz0_ph, _x_sz1_ph, _x_sz2_ph]
    return image, templates_z, raw_scores, scores_up, pos, sizes
...
def _create_siamese(net_path, net_x, net_z, verbose=0):
    ...
    for i in range(_num_layers):
        ...
        if _pool_stride[i] > 0:
            ...
            net_x = tf.nn.max_pool2d(net_x, [1, _pool_sz, _pool_sz, 1],
                                     strides=[1, _pool_stride[i], _pool_stride[i], 1],
                                     padding='VALID', name='pool' + str(i + 1))
            net_z = tf.nn.max_pool2d(net_z, [1, _pool_sz, _pool_sz, 1],
                                     strides=[1, _pool_stride[i], _pool_stride[i], 1],
                                     padding='VALID', name='pool' + str(i + 1))
