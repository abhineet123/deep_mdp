...
class ECO(BaseTracker):
    ...
    def initialize(self, image, bbox: list) -> dict:
        # bbox = info['init_bbox']
        ...
        self.interp_fs = TensorList([dcf.get_interp_fourier(sz, self.params.interpolation_method,
                                                            ...
                                                            self.params.interpolation_windowing, self.params.device) for
                                     sz in self.filter_sz])
        ...
        self.training_samples = TensorList(
            [xf.new_zeros(xf.shape[2], xf.shape[3], self.params.sample_memory_size, cdim, 2) for xf, cdim in
             zip(train_xf, self.compressed_dim)])
        ...
        self.joint_problem = FactorizedConvProblem(self.init_training_samples, self.yf, self.reg_filter,
                                                   self.projection_matrix, self.params, self.init_sample_weights)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        init_samples = self.params.features.extract_transformed(im, self.pos, self.target_scale, self.img_sample_sz,
                                                                transforms)
    ...
    def update_sample_weights(self):
        replace_ind = []
        for sw, prev_ind, num_samp, fparams in zip(self.sample_weights, self.previous_replace_ind,
                                                   self.num_stored_samples, self.fparams):
