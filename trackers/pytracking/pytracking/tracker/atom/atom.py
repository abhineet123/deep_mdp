...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, bbox: list) -> dict:
        ...
        # self.training_samples = self.training_samples.cpu()
        # self.target_feat = self.target_feat.cpu()
    ...
    def track(self, image) -> dict:
        ...
        if self.update_location and self.use_iou_net and flag != 'not_found':
            self.pos = self.pos_iounet.clone()

        # Return new state
        new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        # pos_np = self.pos.detach().cpu().numpy()
        # target_sz_np = self.target_sz.detach().cpu().numpy()
        score_map_np = score_map.detach().cpu().numpy()
        bbox = new_state.numpy()
        out = {
            'bbox': bbox,
    ...
    def set_region(self, frame, bbox):
        assert self.update_location, "set_region cannot be called if update_location is disabled"
        self.pos = torch.Tensor([bbox[1] + (bbox[3] - 1) / 2, bbox[0] + (bbox[2] - 1) / 2])
        self.target_sz = torch.Tensor([bbox[3], bbox[2]])
