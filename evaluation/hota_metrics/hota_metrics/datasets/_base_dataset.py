...
class _BaseDataset(ABC):
    ...
    @classmethod
    def _load_raw_file_mdp(cls, file, num_timesteps, is_gt):
        # Load raw data from text file
        read_data, ignore_data = cls._load_simple_text_file(file)
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                try:
                    ...
                    if is_gt:
                        raise TrackEvalException(
                            'Cannot convert gt data to float. Is data corrupted?')
                    else:
                        raise TrackEvalException(
                            'Cannot convert tracking data to float. Is data corrupted?')
                try:
                    raw_data['dets'][t] = np.atleast_2d(time_data[:, 2:6])
                    raw_data['ids'][t] = np.atleast_1d(time_data[:, 1]).astype(int)
                except IndexError:
                    if is_gt:
                        err = 'Cannot load gt data from file %s, because there is not enough ' \
                              'columns in the data.' % file
                        raise TrackEvalException(err)
                    else:
                        err = 'Cannot load tracker data from file %s, because there is not enough ' \
                              'columns in the data.' % file
                        raise TrackEvalException(err)
                ...
                # if not is_gt:
                #     """all classes be 1 by default"""
                #     raw_data['classes'][t] = np.ones_like(raw_data['ids'][t])
                # else:
                #     raise TrackEvalException(
                #         'GT data is not in a valid format, there is not enough rows in file %s, timestep %i.' % (
                #             file, t))
    ...
    @classmethod
    def get_raw_seq_data_mdp(cls, gt_file, tracked_file, num_timesteps):
        # Load raw data.

        print('reading gt from: {}'.format(gt_file))
        print('reading preds from: {}'.format(tracked_file))

        raw_gt_data = cls._load_raw_file_mdp(gt_file, num_timesteps, is_gt=True)
        raw_tracker_data = cls._load_raw_file_mdp(tracked_file, num_timesteps, is_gt=False)
        raw_data = {**raw_tracker_data, **raw_gt_data}  # Merges dictionaries

        # Calculate similarities for each timestep.
        similarity_scores = []
        for gt_dets_t, tracker_dets_t in zip(raw_data['gt_dets'], raw_data['tracker_dets']):
            ious = cls._calculate_similarities(gt_dets_t, tracker_dets_t)
            similarity_scores.append(ious)
        raw_data['similarity_scores'] = similarity_scores
        return raw_data
