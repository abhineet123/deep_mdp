...
from py3nvml import py3nvml as pynvml3
import torch

print_tensor_sizes = True
last_tensor_sizes = set()
gpu_profile_fn = f'gpu_mem_prof_{datetime.datetime.now():%d%b%y%H%M%S}.txt'

gpu_profile_fn = os.path.abspath(gpu_profile_fn)
    ...
    if event == 'line':
        try:
            ...
            if lineno is not None:
                pynvml3.nvmlInit()
                handle = pynvml3.nvmlDeviceGetHandleByIndex(0)
                meminfo = pynvml3.nvmlDeviceGetMemoryInfo(handle)
                line = linecache.getline(filename, lineno)
                where_str = module_name+' '+func_name+':'+str(lineno)
                txt = f"{where_str:<50}:{meminfo.used/1024**2:<7.1f}Mb {line.rstrip()}\n"
                txt2 = ''
                if print_tensor_sizes is True:
                    ...
                    for t, s, loc in new_tensor_sizes - last_tensor_sizes:
                        txt2 += f'+ {loc:<50} {str(s):<20} {str(t):<10}\n'
            ...
            # print(f'frame.f_globals: {frame.f_globals}')
            pass
